...
class MovieDaoTest {
    ...
    fun testInsertMovies() {
        ...
        movieDao.insertMovies(CacheMovie.fromMovies(duplicates))
    ...
    fun testDeleteMovies() {
        ...
        movieDao.getAllMovies()
            .test()
            .assertValue { it == listOf(CacheMovie.fromMovie(Movie(id = 12345, originalTitle = "Tropic Thunder", genreIds = listOf(1, 2, 3, 4, 5), originalLanguage = "English", adult = true))) }

        movieDao.deleteAll()

        movieDao.getAllMovies()
            .test()
            .assertValue { it == listOf<CacheMovie>() }
    }
}
